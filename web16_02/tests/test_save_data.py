...
class TestSaveData(unittest.TestCase):
    ...
    def test_open(self):
        """ Test open file for save """
        save_applicant_data(applicant, 'fake.csv')
        # assert self.mock_open_file.call_count == 1, 'Открываем файл один раз'
        # assert 'w' in self.mock_open_file.call_args_list[0][0], 'Файл должен быть открыт только для записи'
